...
# Support for NCSim or Xcelium
# If file exists, looks like Xcelium install. Update
# prefix used for tool and library names. Add flag
# to pass to mc_simulator_extensions to signal Xcelium compilation.
ifneq ("$(wildcard $(NC_ROOT)/tools/bin/xmvhdl)","")
  NCTYPE = XMSC
  NCPFX = xm
  XMFLAG = -DXMSC
else
...
ifneq "$(NCSim_EXEC_NCVHDL)" ""
NCVHDL = $(NCSim_EXEC_NCVHDL)
else
NCVHDL = $(NCPFX)vhdl
endif
ifneq "$(NCSim_EXEC_NCVLOG)" ""
NCVLOG = $(NCSim_EXEC_NCVLOG)
else
NCVLOG = $(NCPFX)vlog
endif
ifneq "$(NCSim_EXEC_NCSC)" ""
NCSC = $(NCSim_EXEC_NCSC)
else
NCSC   = $(NCPFX)sc
endif
ifneq "$(NCSim_EXEC_NCSDFC)" ""
NCSDFC = $(NCSim_EXEC_NCSDFC)
else
NCSDFC = $(NCPFX)sdfc
endif
ifneq "$(NCSim_EXEC_NCELAB)" ""
NCELAB = $(NCSim_EXEC_NCELAB)
else
NCELAB = $(NCPFX)elab
endif
ifneq "$(NCSim_EXEC_NCSIM)" ""
NCSIM = $(NCSim_EXEC_NCSIM)
else
NCSIM  = $(NCPFX)sim
endif
...
NCSC_LD_ARGS += -L$(NC_ROOT)/tools/systemc/lib$(LIB64)/gnu$(GCC_VER_DIR) -lsystemc_sh -l$(NCPFX)scCoSim_sh -l$(NCPFX)scCoroutines_sh -l$(NCPFX)sctlm2_sh
...
PWR_EXE_EXT := pwr
  ...
  export VERDI_HOME
  NOVAS_PLATFORM = $(shell NOVAS_EXECUTABLE_ARCH=$(NOVAS_EXECUTABLE_ARCH) $(VERDI_HOME)/bin/novas_plat)
  LINK_LIBPATHS += $(VERDI_HOME)/share/PLI/IUS/$(NOVAS_PLATFORM)
  ...
  LD_LIBRARY_PATH      := $(VERDI_HOME)/share/PLI/IUS/$(NOVAS_PLATFORM):$(VERDI_HOME)/share/PLI/IUS/$(NOVAS_PLATFORM)/boot:$(GCC_HOME)/lib:$(NC_ROOT)/tools/systemc/lib$(LIB64):$(NC_ROOT)/tools/lib$(LIB64):$(NC_ROOT)/tools/tbsc/lib$(LIB64)/gnu$(GCC_VER_DIR):$(LD_LIBRARY_PATH)
...
$(TARGET)/%.cxxts :
	...
	$(NCSC) $(GCC_VER_ARG) -COMPILER $(GCC_COMPILER) $(NC_CFG_ARGS) $(NCSC_OPTS) -CFLAGS "$(GCC_MOPT) -fPIC -o $@.o $(CXX_INC) -DNCSC $(XMFLAG) -c $(NCSC_CXX_OPTS) $(CXXFLAGS) $(CXX_OPTS)" $<
	$(TIMESTAMP)
