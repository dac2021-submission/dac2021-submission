...
SC_MODULE(Source) {
  ...
  void run(){
    ...
    rva_in_src.rw = 1;
    rva_in_src.data = set_bytes<16>("01_00_00_02_00_00_00_01_00_02_00_00_00_00_00_01"); //is_valid=1, mode=0, is_rnn=0, memory_index1=1, memory_index2=0, num_vector= 2, num_output_vector=0, num_timestep=1, num_timestep_padding=00, adpbias_1=2, adpbias_2=0, adpbias_3 = 0, adpbias_4=0
  ...
  void run(){
    ...
    while (1) {
      ...
      if (large_req.PopNB(large_req_dest)) {
         cout << sc_time_stamp() << " - large buffer request sent: " << large_req_dest.memory_index << large_req_dest.vector_index << large_req_dest.timestep_index << endl;
      }
      if (small_req.PopNB(small_req_dest)) {
         cout << sc_time_stamp() << " - small buffer request sent: " << small_req_dest.memory_index << small_req_dest.vector_index << endl;
      }
