...
//
// Description:
//    Provides cordic based implementation of the inverse cosine function for the
...
//      output_type arccos_output;
//      CCS_DESIGN(project)(input, arccos_output);
//      CCS_RETURN (0);
//    }
//    #endif
//
...
namespace ac_math
{
  template <bool b> struct MgcAcIarccosAssert {};
  template <> struct MgcAcIarccosAssert<true> { static void test() {} };

  // Multi-precision approximation of 2*tan(2^-i)/pi
  typedef ac_fixed<128,0,false> val_t;
  static const ac_fixed<128,0,false> x2_acos_pi_pow2_table[] = {
  ...
  template < int ZW >
  static ac_fixed<ZW, 0, false> x2_acos_pi_2mi(int i)
  {
    MgcAcIarccosAssert< ZW <= 128 >::test();
    // Assume no more than 130 entries.
    if (i > 129)
    { return 0; }
    return x2_acos_pi_pow2_table[i];
  }

  //============================================================================
  // Function:  theta = arccos(t)
  // Inputs:
  ...
  void ac_arccos_cordic(ac_fixed<AW,AI,true,AQ,AO> t, ac_fixed<ZW,ZI,false,ZQ,ZO> &arccos)
  {
    const int P_F = 2*(ZW - ZI);
    const int GUARD = 2;
    const int N_I = P_F + 2;
    MgcAcIarccosAssert< P_F <= 128 >::test();
    ...
    for (int i = 0; i < N_I; i++) {
      ...
      dp_t x_d = y >> (i - 1);
      dp_t y_d = x >> (i - 1);
      dp_theta_t theta_d = x2_acos_pi_2mi<P_F>(i);
      dp_t tn_d = tn >> (2*i);
      x -= x >> (2*i);
      y -= y >> (2*i);
      if (sel) {
