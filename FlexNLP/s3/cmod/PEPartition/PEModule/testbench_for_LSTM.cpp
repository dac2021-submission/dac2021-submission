...
SC_MODULE(Source) {
  ...
  void run() {
    ...
    rva_in_src.rw = 1;
    rva_in_src.data = set_bytes<16>("00_00_00_00_00_00_00_00_00_00_40_02_01_00_01_01"); //is_valid=1, is_zero_first=1, is_cluster=0, is_bias=1, num_manager=2, num_output=64 
    ...
    rva_in_src.rw = 1;
    rva_in_src.data = set_bytes<16>("00_00_00_00_00_00_00_20_00_00_00_10_01_02_02_00"); //zero_active=0, adplfloat_bias_weight=2 , adplfloat_bias_bias=2 , adplfloat_bias_input=1 , num_input=16 , base_weight=0 , base_bias=32 (0x0020) , base_input=0 
    ...
    rva_in_src.rw = 1;
    rva_in_src.data = set_bytes<16>("00_00_00_00_00_10_00_60_40_00_00_10_01_02_02_01"); //zero_active=1, adplfloat_bias_weight=2 , adplfloat_bias_bias=2 , adplfloat_bias_input=1 , num_input=16 , base_weight=16384 , base_bias=96 (0x0060) , base_input=16 
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {
        for (int m=0; m < spec::kNumVectorLanes; m++) {
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec.to_rawbits();
           rva_in_src.addr = 0x500000 + (16*(64*i + j) + m + 512)*16;
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {
        for (int m=0; m < spec::kNumVectorLanes; m++) {
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec.to_rawbits();
           rva_in_src.addr = 0x500000 + (16*(64*i + j) + m + 256)*16;
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {
        for (int m=0; m < spec::kNumVectorLanes; m++) {
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec.to_rawbits();
           rva_in_src.addr = 0x500000 + (16*(64*i + j) + m + 768)*16;
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {
        for (int m=0; m < spec::kNumVectorLanes; m++) {
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec.to_rawbits();
           rva_in_src.addr = 0x500000 + (16384 + 16*(64*i + j) + m)*16;
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {
        for (int m=0; m < spec::kNumVectorLanes; m++) {
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec.to_rawbits();
           rva_in_src.addr = 0x500000 + (16384 + 16*(64*i + j) + m + 512)*16;
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {
        for (int m=0; m < spec::kNumVectorLanes; m++) {
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec.to_rawbits();
           rva_in_src.addr = 0x500000 + (16384 + 16*(64*i + j) + m + 256)*16;
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {
        for (int m=0; m < spec::kNumVectorLanes; m++) {
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec.to_rawbits();
           rva_in_src.addr = 0x500000 + (16384 + 16*(64*i + j) + m + 768)*16;
