...
SC_MODULE(testbench) {
  ...
  testbench(sc_module_name name, int cpm)
  : sc_module(name),
  ...
  void run_ilator() {
    ...
    if (interrupted < 2) {
      ...
      if (checkpoint_mode == 1) {
        ...
        check_on = (flex_i.flex_data_in_0 % 10) == 0;
      }
...
int sc_main(int argc, char *argv[]) {
  bool checkpoint_enable = (argc > 1);
  if (!checkpoint_enable) return 0;
  std::istringstream argv_1(argv[1]);
  int checkpoint_mode;
  argv_1 >> checkpoint_mode;
  nvhls::set_random_seed();
  NVINT8 test = 14;
  cout << fixed2float<8, 3>(test) << endl;  

  testbench tb("tb", checkpoint_mode);
