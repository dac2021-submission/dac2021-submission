...
SC_MODULE(Source) {
  ...
  void run() {
    ...
    std::vector<double>  Wx_ref_center, Wh_ref_center;  
    ...
    std::vector<std::vector<double>> Wx_ref, Wx_ref_real, Wh_ref;
    ...
    typedef NVUINTW(4) HalfType;
    typedef typename nvhls::nv_scvector<HalfType, 32> LongHalfVectorType;

    spec::VectorType centers_vec;
    spec::HalfVectorType weight_vec;
    LongHalfVectorType weight_vec2; 
    ...
    rva_in_src.rw = 1;
    rva_in_src.data = set_bytes<16>("00_00_00_00_00_00_00_00_00_00_40_02_01_01_01_01"); //is_valid=1, is_zero_first=1, is_cluster=1, is_bias=1, num_manager=2, num_output=64 
    rva_in_src.addr = set_bytes<3>("40_00_10");  // last 4 bits never used 
    ofstream myfile;
    myfile.open ("axi_commands_for_kmeans_clustering_for_LSTM.csv");
    ...
    rva_in_src.rw = 1;
    rva_in_src.data = set_bytes<16>("00_00_00_00_00_00_00_20_00_00_00_10_02_02_02_00"); //zero_active=0, adplfloat_bias_weight=2 , adplfloat_bias_bias=2 , adplfloat_bias_input=2 , num_input=16 , base_weight=0 , base_bias=32 (0x0020) , base_input=0 
    ...
    rva_in_src.rw = 1;
    rva_in_src.data = set_bytes<16>("00_00_00_00_00_10_00_60_20_00_00_10_02_02_02_01"); //zero_active=1, adplfloat_bias_weight=2 , adplfloat_bias_bias=2 , adplfloat_bias_input=2 , num_input=16 , base_weight=8192 , base_bias=96 (0x0060) , base_input=16 
    rva_in_src.addr = set_bytes<3>("40_00_40");  // last 4 bits never used
    myfile << "2," << "W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
    rva_in.Push(rva_in_src);
    wait(); 

    //Writing cluster centers for pe manager 0
    npy_shape.clear(); npy_data.clear();
    npy::LoadArrayFromNumpy("/group/ttambe/sm6/hls_tapeout/cmod/testbench/dataset_enc256_dec256_unilstm_kmeans/weight_ih_l0_cluster_centers_adaptfloat_kmeans.npy", npy_shape, npy_data);
    cout << "Wx_ref kmeans centers size: " << "- shape0 is: " << npy_shape[0]  << endl;
    Wx_ref_center = npy_data;
    PrintVector(Wx_ref_center); 
    for (unsigned int i=0; i < 16; i++) {
       adpfloat_tmp.Reset();
       adpfloat_tmp.set_value(Wx_ref_center[i], adpbias_weight);
       centers_vec[i] = adpfloat_tmp.to_rawbits();  
       adpfloat_tmp.Reset();
    }
    rva_in_src.rw = 1;
    rva_in_src.data = centers_vec.to_rawbits();
    rva_in_src.addr = set_bytes<3>("40_00_30");  // last 4 bits never used 
    myfile << "2," << "W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
    rva_in.Push(rva_in_src);
    cout << "Pushed Cluster centers for pe manager 0" << endl;
    wait();        

    //Writing cluster centers for pe manager 1
    npy_shape.clear(); npy_data.clear();
    npy::LoadArrayFromNumpy("/group/ttambe/sm6/hls_tapeout/cmod/testbench/dataset_enc256_dec256_unilstm_kmeans/weight_hh_l0_cluster_centers_adaptfloat_kmeans.npy", npy_shape, npy_data);
    cout << "Wh_ref kmeans centers size: " << "- shape0 is: " << npy_shape[0]  << endl;
    Wh_ref_center = npy_data;
    //PrintVector(Wh_ref_center); 
    for (unsigned int i=0; i < 16; i++) {
       adpfloat_tmp.Reset();
       adpfloat_tmp.set_value(Wh_ref_center[i], adpbias_weight);
       centers_vec[i] = adpfloat_tmp.to_rawbits();  
       adpfloat_tmp.Reset();
    }
    rva_in_src.rw = 1;
    rva_in_src.data = centers_vec.to_rawbits();
    rva_in_src.addr = set_bytes<3>("40_00_50");  // last 4 bits never used 
    myfile << "2," << "W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
    rva_in.Push(rva_in_src);
    cout << "Pushed Cluster centers for pe manager 1" << endl;
    wait();  

    //Storing weight_ih_l0 in Weight SRAM
    npy_shape.clear(); npy_data.clear();
    //npy::LoadArrayFromNumpy("/group/ttambe/sm6/hls_tapeout/cmod/testbench/dataset_enc256_dec256_unilstm_kmeans/encoder.rnn_3.weight_ih_l0_cluster_indices.npy", npy_shape, npy_data);
    npy::LoadArrayFromNumpy("/group/ttambe/sm6/hls_tapeout/cmod/testbench/dataset_enc256_dec256_unilstm_kmeans/weight_ih_l0_cluster_indices_float_cluster_indices.npy", npy_shape, npy_data);
    ...
    cout << "Wx_ref[0] matrix is: " << endl;
    PrintVector(Wx_ref[0]);
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {   //chunk of 16 software columns
        for (int m=0; m < 8; m++) { //m is 2rows of software weight
           for (int n=0; n < 2; n++) {
               for (int k=0; k < 16; k++) {
                 float_tmp = Wii_ref[16*i + 2*m +n][16*j + k]; 
                 int int_tmp = static_cast<int>(float_tmp);
                 NVUINT4 int4_tmp = int_tmp; 
                 weight_vec2[16*n+k] = int4_tmp;
                 //cout << weight_vec[k] << endl;
               } // for k
               
            } // for n
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec2.to_rawbits();
           rva_in_src.addr = 0x500000 + (8*(64*i + j) + m)*16;
           myfile << "2," << "W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
           cout << "addresss_wii: " << 8*(64*i + j) + m << endl;
           rva_in.Push(rva_in_src);
           if (rva_in_src.addr ==  0x500000 + (8*(64*15 + 15) + 8)*16) {
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {   //chunk of 32 software columns
        for (int m=0; m < 8; m++) { //m is 2rows of software weight
           for (int n=0; n < 2; n++) {
               for (int k=0; k < 16; k++) {
                 float_tmp = Wif_ref[16*i + 2*m +n][16*j + k]; 
                 int int_tmp = static_cast<int>(float_tmp);
                 NVUINT4 int4_tmp = int_tmp; 
                 weight_vec2[16*n+k] = int4_tmp;
               } // for k
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec2.to_rawbits();
           rva_in_src.addr = 0x500000 + (8*(64*i + j) + m + 256)*16;
           myfile << "2," << "W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
           cout << "addresss_wif: " << 8*(64*i + j) + m + 256 << endl;
           rva_in.Push(rva_in_src);
           if (rva_in_src.addr ==  0x500000 + (8*(64*15 + 15) + 8 +256)*16) {
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {   //chunk of 32 software columns
        for (int m=0; m < 8; m++) { //m is 2rows of software weight
           for (int n=0; n < 2; n++) {
               for (int k=0; k < 16; k++) {
                 float_tmp = Wig_ref[16*i + 2*m +n][16*j + k]; 
                 int int_tmp = static_cast<int>(float_tmp);
                 NVUINT4 int4_tmp = int_tmp;
                 weight_vec2[16*n+k] = int4_tmp;
               } // for k
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec2.to_rawbits();
           rva_in_src.addr = 0x500000 + (8*(64*i + j) + m + 128)*16;
           myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
           cout << "addresss_wig: " << 8*(64*i + j) + m + 128 << endl;
           rva_in.Push(rva_in_src);
           if (rva_in_src.addr ==  0x500000 + (8*(64*15 + 15) + 8 + 128)*16) {
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {   //chunk of 32 software columns
        for (int m=0; m < 8; m++) { //m is 2rows of software weight
           for (int n=0; n < 2; n++) {
               for (int k=0; k < 16; k++) {
                 float_tmp = Wio_ref[16*i + 2*m +n][16*j + k]; 
                 int int_tmp = static_cast<int>(float_tmp);
                 NVUINT4 int4_tmp = int_tmp;
                 weight_vec2[16*n+k] = int4_tmp;
               } // for k
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec2.to_rawbits();
           rva_in_src.addr = 0x500000 + (8*(64*i + j) + m + 384)*16;
           myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
           cout << "addresss_wio: " << 8*(64*i + j) + m + 384 << endl;
           rva_in.Push(rva_in_src);
           if (rva_in_src.addr ==  0x500000 + (8*(64*15 + 15) + 8 + 384)*16) {
    ...
    npy_shape.clear(); npy_data.clear();
    //npy::LoadArrayFromNumpy("/group/ttambe/sm6/hls_tapeout/cmod/testbench/dataset_enc256_dec256_unilstm_kmeans/encoder.rnn_3.weight_hh_l0_cluster_indices.npy", npy_shape, npy_data);
    npy::LoadArrayFromNumpy("/group/ttambe/sm6/hls_tapeout/cmod/testbench/dataset_enc256_dec256_unilstm_kmeans/weight_ih_l0_cluster_indices_float_cluster_indices.npy", npy_shape, npy_data);
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {   //chunk of 32 software columns
        for (int m=0; m < 8; m++) { //m is 2rows of software weight
           for (int n=0; n < 2; n++) {
               for (int k=0; k < 16; k++) {
                 float_tmp = Whi_ref[16*i + 2*m +n][16*j + k]; 
                 int int_tmp = static_cast<int>(float_tmp);
                 NVUINT4 int4_tmp = int_tmp;
                 weight_vec2[16*n+k] = int4_tmp;
               } // for k
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec2.to_rawbits();
           rva_in_src.addr = 0x500000 + (8192 + 8*(64*i + j) + m)*16;
           myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
           cout << "addresss_whi: " << 8192 + 8*(64*i + j) + m << endl;
           rva_in.Push(rva_in_src);
           if (rva_in_src.addr ==  0x500000 + (8192 + 8*(64*15 + 15) + 8)*16) {
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {   //chunk of 32 software columns
        for (int m=0; m < 8; m++) { //m is 2rows of software weight
           for (int n=0; n < 2; n++) {
               for (int k=0; k < 16; k++) {
                 float_tmp = Whf_ref[16*i + 2*m +n][16*j + k]; 
                 int int_tmp = static_cast<int>(float_tmp);
                 NVUINT4 int4_tmp = int_tmp;
                 weight_vec2[16*n+k] = int4_tmp;
               } // for k
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec2.to_rawbits();
           rva_in_src.addr = 0x500000 + (8192 + 8*(64*i + j) + m +256)*16;
           myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
           cout << "addresss_whf: " << 8192 + 8*(64*i + j) + m + 256 << endl;
           rva_in.Push(rva_in_src);
           if (rva_in_src.addr ==  0x500000 + (8192 + 8*(64*15 + 15) + 8 + 255)*16) {
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {   //chunk of 32 software columns
        for (int m=0; m < 8; m++) { //m is 2rows of software weight
           for (int n=0; n < 2; n++) {
               for (int k=0; k < 16; k++) {
                 float_tmp = Whg_ref[16*i + 2*m +n][16*j + k]; 
                 int int_tmp = static_cast<int>(float_tmp);
                 NVUINT4 int4_tmp = int_tmp;
                 weight_vec2[16*n+k] = int4_tmp;
               } // for k
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec2.to_rawbits();
           rva_in_src.addr = 0x500000 + (8192 + 8*(64*i + j) + m + 128)*16;
           myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
           cout << "addresss_whg: " << 8192 + 8*(64*i + j) + m + 128 << endl;
           rva_in.Push(rva_in_src);
           if (rva_in_src.addr ==  0x500000 + (8192 + 8*(64*15 + 15) + 8 + 128)*16) {
    ...
    for (unsigned int i=0; i < 16; i++) {  
      for (int j=0; j < 16; j++) {   //chunk of 32 software columns
        for (int m=0; m < 8; m++) { //m is 2rows of software weight
           for (int n=0; n < 2; n++) {
               for (int k=0; k < 16; k++) {
                 float_tmp = Who_ref[16*i + 2*m +n][16*j + k];
                 int int_tmp = static_cast<int>(float_tmp);
                 NVUINT4 int4_tmp = int_tmp;
                 weight_vec2[16*n+k] = int4_tmp;
               } // for k
           ...
           rva_in_src.rw = 1;
           rva_in_src.data = weight_vec2.to_rawbits();
           rva_in_src.addr = 0x500000 + (8192 + 8*(64*i + j) + m + 384)*16;
           myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
           cout << "addresss_who: " << 8192 + 8*(64*i + j) + m + 384 << endl;
           rva_in.Push(rva_in_src);
           if (rva_in_src.addr ==  0x500000 + (8192 + 8*(64*15 + 15) + 8 + 384)*16) {
    ...
    //bias_ih
    npy_shape.clear(); npy_data.clear();
    npy::LoadArrayFromNumpy("/group/ttambe/sm6/hls_tapeout/cmod/testbench/dataset_enc256_dec256_unilstm_kmeans/real_compressed_weights/encoder.rnn_3.bias_ih_l0adaptfloat_kmeans.npy", npy_shape, npy_data);
    ...
    for (unsigned int i=0; i < 16; i++) { 
      ...
      rva_in_src.addr = 0x600000 + (32 + 4*i)*16;
      myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
    ...
    for (unsigned int i=0; i < 16; i++) { 
      ...
      rva_in_src.rw = 1;
      rva_in_src.data = bias_vec.to_rawbits();
      rva_in_src.addr = 0x600000 + (32 + 4*i + 2)*16;
      myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
    ...
    for (unsigned int i=0; i < 16; i++) { 
      ...
      rva_in_src.rw = 1;
      rva_in_src.data = bias_vec.to_rawbits();
      rva_in_src.addr = 0x600000 + (32 + 4*i + 1)*16;
      myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
    ...
    for (unsigned int i=0; i < 16; i++) { 
      ...
      rva_in_src.rw = 1;
      rva_in_src.data = bias_vec.to_rawbits();
      rva_in_src.addr = 0x600000 + (32 + 4*i + 3)*16;
      myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
      cout << "addresss_bio: " << 32 + 4*i + 3 << endl;
      rva_in.Push(rva_in_src);
    ...
    //bias_hh
    npy_shape.clear(); npy_data.clear();
    npy::LoadArrayFromNumpy("/group/ttambe/sm6/hls_tapeout/cmod/testbench/dataset_enc256_dec256_unilstm_kmeans/real_compressed_weights/encoder.rnn_3.bias_hh_l0adaptfloat_kmeans.npy", npy_shape, npy_data);
    ...
    for (unsigned int i=0; i < 16; i++) { 
      ...
      rva_in_src.rw = 1;
      rva_in_src.data = bias_vec.to_rawbits();
      rva_in_src.addr = 0x600000 + (96 + 4*i)*16;
      myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
      cout << "addresss_bhi: " << 96 + 4*i << endl;
      rva_in.Push(rva_in_src);
    ...
    for (unsigned int i=0; i < 16; i++) { 
      ...
      rva_in_src.rw = 1;
      rva_in_src.data = bias_vec.to_rawbits();
      rva_in_src.addr = 0x600000 + (96 + 4*i + 2)*16;
      myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
    ...
    for (unsigned int i=0; i < 16; i++) { 
      ...
      rva_in_src.rw = 1;
      rva_in_src.data = bias_vec.to_rawbits();
      rva_in_src.addr = 0x600000 + (96 + 4*i + 1)*16;
      myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
    ...
    for (unsigned int i=0; i < 16; i++) { 
      ...
      rva_in_src.rw = 1;
      rva_in_src.data = bias_vec.to_rawbits();
      rva_in_src.addr = 0x600000 + (96 + 4*i + 3)*16;
      myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
    ...
    //PrintVector(Xt_ref);
    for (unsigned int i=0; i < (npy_shape[2] / 16); i++) {  
        for (int j=0; j < spec::kNumVectorLanes; j++) {
           adpfloat_tmp.Reset();
      ...
      myfile << "2" << ",W," << hex << rva_in_src.addr << "," << hex << rva_in_src.data << "\n";
      rva_in.Push(rva_in_src);
      if (rva_in_src.addr ==  0x600000 + ((npy_shape[2]/16) - 1)*16) {
    ...
    //ActUnit Axi Config
    rva_in_src.rw = 1;
    rva_in_src.data = set_bytes<16>("00_00_00_00_00_00_00_00_00_00_00_10_18_02_01_01"); //is_valid=1, is_zero_first=1, adpfloat_bias=2, num_inst=24, num_output=16, buffer/output addr_base=0
