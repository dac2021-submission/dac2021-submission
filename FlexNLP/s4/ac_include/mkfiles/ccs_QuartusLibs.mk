...
### *** Quartus Shell - the change to generate the command script, command file, and comment out the mkdir commands 
###                     was required to work-around vlib failure (vlib-35) Failed to create directory error.  Verify 
###                     with Questa 2019.2 or later to see if this can be removed.
	@$(QSHELL) --simlib_comp -tool $(SIMULATOR) -family all -language verilog -tool_path $(SIM_PATH) -directory $(PCL_CACHE) -rtl_only -gen_only -cmd_file ./comp_v.tcl -suppress_messages
	@sed -e 's/file mkdir/#file mkdir/' -i $(PCL_CACHE)/comp_v.tcl
	@$(MKDIR) $(subst /,$(PATHSEP),$(PCL_CACHE)/verilog_libs)
	@$(VSIM) -c -do $(PCL_CACHE)/comp_v.tcl -logfile .vsim_v.log 
	@$(QSHELL) --simlib_comp -tool $(SIMULATOR) -family all -language vhdl -tool_path $(SIM_PATH) -directory $(PCL_CACHE) -rtl_only -gen_only -cmd_file ./comp_vhd.tcl -suppress_messages
	@sed -e 's/file mkdir/#file mkdir/' -i $(PCL_CACHE)/comp_vhd.tcl 
	@$(MKDIR) $(subst /,$(PATHSEP),$(PCL_CACHE)/vhdl_libs)
	@$(VSIM) -c -do $(PCL_CACHE)/comp_vhd.tcl -logfile .vsim_vhd.log
