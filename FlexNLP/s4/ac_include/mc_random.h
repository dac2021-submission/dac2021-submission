...
#if defined(__AC_STD_FLOAT_H) && !defined(MC_RANDOM_H_INC__AC_STD_FLOAT_H)
#define MC_RANDOM_H_INC__AC_STD_FLOAT_H
template<int W, int E>
inline void mc_random(ac_std_float<W,E> &x, bool finite=true) {
  // finite == true => NaN and Inf are excluded
  ac_int<W,true> x_i;
  mc_random(x_i);
  int exp = x_i.template slc<E>(W-E-1).to_int();
  if(exp == -1)
    x_i[W-3] = false;
  x.set_data(x_i);
}
template<ac_ieee_float_format Format>
inline void mc_random(ac_ieee_float<Format> &x, bool finite=true) {
  // finite == true => NaN and Inf are excluded
  ac_std_float<ac_ieee_float<Format>::width, ac_ieee_float<Format>::e_width> t;
  mc_random(t,finite);
  x = t;
}
inline void mc_random(ac::bfloat16 &x, bool finite=true) {
  // finite == true => NaN and Inf are excluded
  ac_std_float<16,8> t;
  mc_random(t,finite);
  x = t;
}
#endif // MC_RANDOM_H_INC__AC_STD_FLOAT_H
