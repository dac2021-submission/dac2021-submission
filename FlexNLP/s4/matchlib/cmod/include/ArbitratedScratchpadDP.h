...
class ArbitratedScratchpadDP {
  ...
  public:
  /*yxdbg*/
  WordType GetData(Address a) {
    BankIndex bidx = GetBankIndex(a);
    LocalIndex lidx = GetLocalIndex(a);

    return banks.read(lidx, bidx);
  }

  void SetData(Address a, WordType d) {
    BankIndex bidx = GetBankIndex(a);
    LocalIndex lidx = GetLocalIndex(a);

    banks.write(lidx, bidx, d);
    return;
  }
