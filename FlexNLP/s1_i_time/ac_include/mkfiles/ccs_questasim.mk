...
ifeq "$(QuestaSIM_SYSC_VERSION)" "default"
MSIM_SYSC_VERSION := 
endif
ifeq "$(QuestaSIM_SYSC_VERSION)" "2.2"
MSIM_SYSC_VERSION := -sc22
$(warning Warning: SystemC-2.2 version support is being deprecated.)
endif
ifeq "$(QuestaSIM_SYSC_VERSION)" "2.3"
MSIM_SYSC_VERSION := 
endif

MODEL_TECH       := $(QuestaSIM_Path)
ENABLE_VOPT      := $(QuestaSIM_ENABLE_VOPT)
ENABLE_QWAVE     := $(QuestaSIM_ENABLE_QWAVE)
LP_SAT           := $(LowPower_SWITCHING_ACTIVITY_TYPE)
VOPT_ARGS        := $(QuestaSIM_VOPT_ARGS) $(MSIM_SYSC_VERSION)
VCOM_OPTS        = $(QuestaSIM_VCOM_OPTS)
VLOG_OPTS        += $(QuestaSIM_VLOG_OPTS)
...
SCCOM_OPTS       = $(QuestaSIM_SCCOM_OPTS) $(MSIM_SYSC_VERSION)
FORCE_32BIT      = $(QuestaSIM_FORCE_32BIT)
VSIM_OPTS        = $(QuestaSIM_VSIM_OPTS) $(MSIM_SYSC_VERSION)
GATE_VSIM_OPTS   = $(QuestaSIM_GATE_VSIM_OPTS)
MSIM_DOFILE      = $(QuestaSIM_MSIM_DOFILE)
DEF_MODELSIM_INI = $(QuestaSIM_DEF_MODELSIM_INI)
VCD_SIZE_LIMIT   = $(QuestaSIM_VCD_SIZE_LIMIT)
QUESTA_DISABLE_WLF = $(QuestaSIM_SUPPRESS_WAVEFORMS)
...
ENABLE_CODE_COVERAGE = $(QuestaSIM_ENABLE_CODE_COVERAGE)
QHOME = $(QuestaSIM_QHOME)
...
# Options for SystemVerilog Assertion support in SCVerify
BIND_SVA_OPTS = -sv -mfcu -cuname $(SVA_DU)
ifneq "$(SVA_DU)" ""
OPTIONAL_DU += $(SVA_DU)
endif
...
ifeq "$(MODEL_TECH)" ""
   $(error No QuestaSIM installation found. Check your QuestaSIM flow Path option in Catapult)
endif
...
ifneq "$(OS)" "Windows_NT"
  ...
  ifeq "$(OS_NAME)" "Ubuntu"
  ifeq "$(wildcard $(MODEL_TECH)/../gcc-*/bin/gcc.real)" ""
    # Use the system linker 
    $(warning Warning: Using QuestaSim on Ubuntu requires -B/usr/bin link argument)
    ifeq "$(MTI_VCO_MODE)" "32"
      $(warning Warning: Forcing 32-bit mode on Ubuntu requires patching QuestaSim gcc installation, contact support.)
    endif
    endif
  endif
endif
...
ifneq "$(QuestaSIM_EXEC_VCOM)" ""
VCOM       = $(QuestaSIM_EXEC_VCOM)
else
VCOM       = $(MODEL_TECH)/vcom
endif
ifneq "$(QuestaSIM_EXEC_VLOG)" ""
VLOG       = $(QuestaSIM_EXEC_VLOG)
else
VLOG       = $(MODEL_TECH)/vlog
endif
ifneq "$(QuestaSIM_EXEC_SCCOM)" ""
SCCOM      = $(QuestaSIM_EXEC_SCCOM)
else
SCCOM      = $(MODEL_TECH)/sccom
endif
ifneq "$(QuestaSIM_EXEC_VOPT)" ""
VOPT       = $(QuestaSIM_EXEC_VOPT)
else
VOPT       = $(MODEL_TECH)/vopt
endif
ifneq "$(QuestaSIM_EXEC_VSIM)" ""
VSIM       = $(QuestaSIM_EXEC_VSIM)
else
...
ifeq "$(QuestaSIM_ALLOW_DUP_SYMS)" "true"
...
# QuestaSim always use GNU C++ compiler
...
ifeq "$(QuestaSIM_MSIM_AC_TYPES)" "true"
...
export VERDI_HOME
ifeq "$(MTI_VCO_MODE)" "32"
NOVAS_LIB_DIR = $(VERDI_HOME)/share/PLI/MODELSIM/LINUX
else
NOVAS_LIB_DIR = $(VERDI_HOME)/share/PLI/MODELSIM/LINUX64
endif
LD_LIBRARY_PATH := $(NOVAS_LIB_DIR)$(if $(LD_LIBRARY_PATH),:$(LD_LIBRARY_PATH))
export LD_LIBRARY_PATH
VSIM_OPTS += -pli $(NOVAS_LIB_DIR)/novas_fli.so
endif
...
# add dependency on .ccs_env_opts/QuestaSIM_ENABLE_CODE_COVERAGE.ts
...
$(TMP_CXX_SRC): .ccs_env_opts/QuestaSIM_MSIM_AC_TYPES.ts
...
view_wave: MODELSIM=$(PROJ2WORK)$(PATHSEP)$(TARGET)$(PATHSEP)modelsim.ini
view_wave: SCVLIBS=$(PROJ2WORK)
# autocheck and covercheck are run from the solution directory
autocheck/autocheck_verify.db.ts: MODELSIM=$(TARGET)$(PATHSEP)modelsim.ini
autocheck/autocheck_verify.db.ts: SCVLIBS=.
covercheck/covercheck_verify.db.ts: MODELSIM=$(TARGET)$(PATHSEP)modelsim.ini
covercheck/covercheck_verify.db.ts: SCVLIBS=.
...
$(TARGET)/scverify_msim_wave.tcl: $(WORK2SOLN)/scverify/ccs_wave_signals.dat
	...
	$(TCLSH_CMD) $(MGC_HOME)/pkgs/sif/userware/En_na/flows/app_questasim.flo create_wave_script $(PROJ2SOLN)/scverify/ccs_wave_signals.dat $@ $(DUTINSTINFOFILE) $(QWAVE_SIGNAL_FILE) $(MSIM_DOFILE)
else
$(TARGET)/scverify_msim_wave.tcl:
	$(TCLSH_CMD) $(MGC_HOME)/pkgs/sif/userware/En_na/flows/app_questasim.flo create_wave_script "" $@ $(DUTINSTINFOFILE) $(QWAVE_SIGNAL_FILE) $(MSIM_DOFILE)
endif
...
view_wave:
	-@echo "============================================"
	-@echo "Viewing wavefile '$(WLFFILE)'"
	-@$(CD) $(WORK2PROJ)$(;) test ! -e $(WLFFILE) && echo "Wavefile '$(WLFFILE)' not found" || \
	"$(VSIM)" -i -view $(WLFFILE) -nolog -do "do {$(CMDFILE)}"
...
autocheck/autocheck_verify.db.ts: ac_mkdir $(TARGET)/make_libs $(TMP_VHDL_SRC) $(TMP_VLOG_SRC)
	-@echo "============================================"
	...
	-@echo "exit" >>run_ac.do
	$(QVERIFY) -c -licq -od autocheck -do run_ac.do
	$(TIMESTAMP)
  

# End-user targets
autocheck: autocheck/autocheck_verify.db.ts

autocheck_gui: autocheck/autocheck_verify.db.ts
	-@echo "============================================"
	-@echo "Running Autocheck GUI on: $(TOP_HDL_ENTITY) with db $(<:.ts=)"
	$(QVERIFY) $(<:.ts=)

#=============================================================================================
# UCDB and Covercheck targets are only available for RTL and only if the options are enabled
ifeq "$(STAGE)" "rtl"
ifeq "$(QuestaSIM_ENABLE_CODE_COVERAGE)" "true"
...
# This is the same action as the sim target using build target prereqs
$(TARGET)/$(TOP_HDL_ENTITY).ucdb: $(TARGET)/make_libs $(TARGET)/scverify_msim_wave.tcl $(TARGET)/$(TOP_DU)vopts
	$(CD) $(WORK2PROJ)$(;) "$(VSIM)" $(VSIM_OPTS) -l "$(LOGFILE)" $(VLOG_LIBS) $(TOP_DU_SIM) $(SIM_OPTS) $(SDF_OPT) $(SC_ARGS) -c -do "do {$(CMDFILE)}"

ifeq "$(QuestaSIM_WITNESS_WAVEFORM)" "true"
...
covercheck/covercheck_verify.db.ts: cc_mkdir $(TARGET)/$(TOP_HDL_ENTITY).ucdb .ccs_env_opts/QuestaSIM_WITNESS_WAVEFORM.ts .ccs_env_opts/QuestaSIM_COVERCHECK_TIMEOUT.ts .ccs_env_opts/QuestaSIM_COVERCHECK_TCL.ts
	-@echo "============================================"
	...
	-@echo "if {[file exists [file join $(QuestaSIM_COVERCHECK_TCL) covercheck_init.tcl]]} { do [file join $(QuestaSIM_COVERCHECK_TCL) covercheck_init.tcl] }" >>run_cc.do
	-@echo "covercheck compile -d $(TOP_HDL_ENTITY) -work work -modelsimini $(MODELSIM) $(COV_LIBS)" >>run_cc.do
	-@echo "if {[file exists [file join $(QuestaSIM_COVERCHECK_TCL) covercheck_postcompile.tcl]]} { do [file join $(QuestaSIM_COVERCHECK_TCL) covercheck_postcompile.tcl] }" >>run_cc.do
	-@echo "covercheck load ucdb $(TARGET)/$(TOP_HDL_ENTITY).ucdb" >>run_cc.do
	-@echo "if {[file exists [file join $(QuestaSIM_COVERCHECK_TCL) covercheck_postload.tcl]]} { do [file join $(QuestaSIM_COVERCHECK_TCL) covercheck_postload.tcl] }" >>run_cc.do
	-@echo "covercheck verify -mergedesigndiffs $(WITNESS) -timeout $(QuestaSIM_COVERCHECK_TIMEOUT)" >>run_cc.do
	-@echo "if {[file exists [file join $(QuestaSIM_COVERCHECK_TCL) covercheck_postverify.tcl]]} { do [file join $(QuestaSIM_COVERCHECK_TCL) covercheck_postverify.tcl] }" >>run_cc.do
...
# End-user targets
covercheck: covercheck/covercheck_verify.db.ts

covercheck_gui: covercheck/covercheck_verify.db.ts
	-@echo "============================================"
	-@echo "Running Covercheck GUI on: '$(TOP_HDL_ENTITY)' with db $(<:.ts=)"
	$(QVERIFY) $(<:.ts=)
...
helptext:
	...
	@-$(ECHO) "   view_wave  Open an existing wlf waveform file"
