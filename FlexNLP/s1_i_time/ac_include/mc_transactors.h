...
public:
  ...
  // Called on every rising edge of sizelz signal of mgc_chan_in/ccs_in_wait_szchan component
...
{
public:
  void bind_clk(sc_signal_in_if<bool>& clk, bool phase, sc_signal_in_if<sc_logic>& sw_rst) {
    _clk.bind(clk);
    _phase = phase;
    _sw_rst.bind(sw_rst);
  }
  bool sw_reset_active() { return _sw_rst.read() == SC_LOGIC_1; }
...
protected:
  ...
  struct objblock {
    ...
    void transfer(access_fct fct, int brh, int brl, IT& val) {
      sc_assert(_fbr <= brl);   // lower row bounds check
      sc_assert(_lbr >= brh);   // upper row bounds check
      for (; brl <= brh; ++brl) {
    ...
    const int                  _step;  // interleave (column) splitting value
    const int                  _ch;    // column high (msb)  |
    const int                  _cl;    // column low  (lsb)  |-- word in row
    const int                  _cc;    // column width       |
  ...
  IT& get_value(int transno) {
    ...
    while (int(_values.size()) + _trans_begin <= transno) {
      // Add a new vector to the queue. Inputs get tb data and convert from c++ type to vector 
  ...
  // Initialize the sc_lv value vector for use in transactors
  //  called from this->get_value(int) 
  // Output transactors use this base to initialize the vector
  // Input transactors use a virtual override first to get captured tb data and convert from c++ type to the vector
  virtual void receive_value_from_tb() { 
    _values.push_back(IT('0')); 
  }

  // Remove the used sc_lv value vector and increment the trans_begin head counter 
  //  called from this->exchange_value()
  // Input transactors use this base to just remove the vector 
  // Output transactors use a virtual override first to convert the vector to c++ type and put it in a tb fifo for compare
  ...
  // Transactor common var metrics
  // -A transactor with multiple registered objblocks:
  //   has split resources (1 var -> N rsc)
  //  The var row and possibly word metrics will be carved up for each objblock
  //
  // -A transactor with one registered objblock:
  //   has a one to one mapping (1 var -> 1 rsc)
  //   or is one of many transactors merged to a common resource (N var -> 1 rsc)
  //  The var metrics then determine relative position within the resource
  const int _CH;              // Column High   (word msb)
  const int _BaseBit;         // Column offset (word lsb)
  const int _WordWidth; 
  const int _RH;              // Row High   (last,  high, most significant row)
  const int _BaseAddr;        // Row offset (first, low,  least significant row)
      ...
      if ((blk->_triosy->read() == SC_LOGIC_1) || _pending_triosy) {  // only valid at active clock edge
        if (blk->_mc_rsc_block->bus_transfer_required()) {
          ...
          if (blk->_trans++ == _trans_begin) {
            ...
            if (send_value) {
              // Remove the used vector from the queue. Outputs first convert the vector to c++ type and push to tb
              send_value_to_tb();
            }
          }
...
protected:
  ...
  virtual void initiate_driving_value_adjustments(int blk_id, int RH, int BaseAddr, int CH, int BaseBit) {
    ...
    if (!this->sw_reset_active() && !this->_empty && blk->_triosy->read() == SC_LOGIC_1
        && CH >= this->_BaseBit && BaseBit <= this->_CH && RH >= blk->_fbr && BaseAddr <= blk->_lbr) {
  ...
  virtual void send_value_to_objblock(objblock* blk) {
    ...
    if (!this->_empty) {
      ...
      if (this->log_event(MC_TRANSACTOR_READ)) {
        ...
        msg << "MC_TRANSACTOR_READ - sending value #" << blk->_trans << " to '" << blk->_name << "' value: " << t << " @ " << sc_time_stamp();
  ...
  virtual void receive_value_from_tb() {
    if (this->sw_reset_active() && _ld_count > 0) return;
    std::ostringstream msg;
...
protected:
  ...
  void receive_value_from_objblock(objblock* blk) {
    ...
    if (this->log_event(MC_TRANSACTOR_WRITE)) {
      ...
      vector_to_type(v, Tsigned, &t);
      std::ostringstream msg;
      msg << "MC_TRANSACTOR_WRITE - received value #" << blk->_trans << " from '" << blk->_name << "' : " << t << " @ " << sc_time_stamp();
      SC_REPORT_INFO(this->name(), msg.str().c_str());
    }
  }
  ...
  virtual void send_value_to_objblock(objblock* blk) {
    ...
    if (!this->_empty) {
      ...
      if (this->log_event(MC_TRANSACTOR_READ)) {
        ...
        vector_to_type(v, Tsigned, &t);
        std::ostringstream msg;
        msg << "MC_TRANSACTOR_READ - sending value #" << blk->_trans << " to '" << blk->_name << "' : " << t << " @ " << sc_time_stamp();
        SC_REPORT_INFO(this->name(), msg.str().c_str());
      }
    }
  }
