...
#if defined(__AC_STD_FLOAT_H)
// Conversion for ieee floating pt
template <ac_ieee_float_format Format>
void simtype_to_origtype(SimStruct *S,
                         int loc, 
                         const void *untyped_vp, 
                         const fxpStorageContainerCategory sc, 
                         ac_ieee_float<Format> &target)
{
  switch(sc) {
  case FXP_STORAGE_UINT8: {
    ssSetErrorStatus(S,"FXP_STORAGE_UINT8 not supported in simtype_to_origtype");
    break;
  }
  case FXP_STORAGE_INT8: {
    ssSetErrorStatus(S,"FXP_STORAGE_INT8 not supported in simtype_to_origtype");
    break;
  }
  case FXP_STORAGE_UINT16: {
    ssSetErrorStatus(S,"FXP_STORAGE_UINT16 not supported in simtype_to_origtype");
    break;
  }
  case FXP_STORAGE_INT16: {
    ssSetErrorStatus(S,"FXP_STORAGE_INT16 not supported in simtype_to_origtype");
    break;
  }
  case FXP_STORAGE_UINT32: {
    ssSetErrorStatus(S,"FXP_STORAGE_UINT32 not supported in simtype_to_origtype");
    break;
  }
  case FXP_STORAGE_INT32: {
    ssSetErrorStatus(S,"FXP_STORAGE_INT32 not supported in simtype_to_origtype");
    break;
  }
  case FXP_STORAGE_UNKNOWN: {
    ssSetErrorStatus(S,"FXP_STORAGE_UNKNOWN not supported in simtype_to_origtype");
  ...
  case FXP_STORAGE_DOUBLE: {
    const real64_T *typed_p = static_cast<const real64_T*>(untyped_vp);
    double tmpDbl = typed_p[loc];
    ac_ieee_float<Format> tmpIeee(tmpDbl);
    target = tmpIeee;
  } break;
  case FXP_STORAGE_SINGLE: {
    const real32_T *typed_p = static_cast<const real32_T*>(untyped_vp);
    float tmpFlt = typed_p[loc];
    ac_ieee_float<Format> tmpIeee(tmpFlt);
    target = tmpIeee;
  } break;
  case FXP_STORAGE_CHUNKARRAY: {
    ssSetErrorStatus(S,"FXP_STORAGE_CHUNKARRAY not supported in simtype_to_origtype");
   ...
   switch(dt) {
      ...
      default: {
         switch(sc) {
            ...
            case FXP_STORAGE_UNKNOWN: {
               ssSetErrorStatus(S,"FXP_STORAGE_UNKNOWN Right Here not supported in origtype_to_simtype");
               break;
            }
            case FXP_STORAGE_DOUBLE: {
               ssSetErrorStatus(S,"FXP_STORAGE_DOUBLE not supported in origtype_to_simtype");
...
#if defined(__AC_STD_FLOAT_H)
template <ac_ieee_float_format Format>
void origtype_to_simtype(SimStruct *S, 
                         int loc, 
                         void *untyped_vp, 
                         fxpStorageContainerCategory sc, 
                         ac_ieee_float<Format> &src)
{
  switch(sc) {
  case FXP_STORAGE_UINT8: {
    ssSetErrorStatus(S,"FXP_STORAGE_UINT8 not supported in origtype_to_simtype");
    break;
  }
  case FXP_STORAGE_INT8: {
    ssSetErrorStatus(S,"FXP_STORAGE_INT8 not supported in origtype_to_simtype");
    break;
  }
  case FXP_STORAGE_UINT16: {
    ssSetErrorStatus(S,"FXP_STORAGE_UINT16 not supported in origtype_to_simtype");
    break;
  }
  case FXP_STORAGE_INT16: {
    ssSetErrorStatus(S,"FXP_STORAGE_INT16 not supported in origtype_to_simtype");
    break;
  }
  case FXP_STORAGE_UINT32: {
    ssSetErrorStatus(S,"FXP_STORAGE_UINT32 not supported in origtype_to_simtype");
    break;
  }
  case FXP_STORAGE_INT32: {
    ssSetErrorStatus(S,"FXP_STORAGE_INT32 not supported in origtype_to_simtype");
    break;
  }
  case FXP_STORAGE_UNKNOWN: {
    ssSetErrorStatus(S,"FXP_STORAGE_UNKNOWN not supported in origtype_to_simtype");
  ...
  case FXP_STORAGE_DOUBLE: {
    real64_T *typed_p = static_cast< real64_T*>(untyped_vp);
    ac_std_float<src.width, src.e_width> stdF = src.to_ac_std_float();
    double tmpDbl = stdF.to_double();
    typed_p[loc] = tmpDbl;
    break;
  }
  case FXP_STORAGE_SINGLE: {
    real32_T *typed_p = static_cast< real32_T*>(untyped_vp);
    ac_std_float<src.width, src.e_width> stdF = src.to_ac_std_float();
    float tmpFlt = stdF.to_float();
    typed_p[loc] = tmpFlt;
    break;
  }
  case FXP_STORAGE_CHUNKARRAY: {
   ...
   ac_ieee_float<Format> &target)
{
   ...
   simtype_to_origtype(S,loc,untyped_vp, input_sc, target);
}
#endif
   ...
   ac_ieee_float<Format> &target)
{
  ...
  // now copy data
  simtype_to_origtype(S, loc, (void*)fieldP, input_sc, target);
...
void convert_input(
   ...
   int loc,
   int PortIdx,
   ac_complex< ac_ieee_float<Format> > &target)
{
   // Create untyped void ptr to input data port
   const void *untyped_vp = ssGetInputPortSignal  (S, PortIdx);
   ...
   // now copy REAL data
   ac_ieee_float<Format> tmp_real;
   simtype_to_origtype(S,loc,untyped_vp, input_sc, tmp_real);
   target.set_r(tmp_real);
   // now copy IMAG data
   ac_ieee_float<Format> tmp_imag;
   simtype_to_origtype(S,loc+1,untyped_vp, input_sc, tmp_imag);
   target.set_i(tmp_imag);
}
#endif
   ...
   ac_complex<ac_ieee_float<Format> > &target)
{
  ...
  ac_ieee_float<Format> tmp_real;
  ac_ieee_float<Format> tmp_imag;
  simtype_to_origtype(S, loc, (void*)fieldP, input_sc, tmp_real);
  simtype_to_origtype(S, loc+1, (void*)fieldP, input_sc, tmp_imag);
  target.set_r(tmp_real);
  target.set_i(tmp_imag);
   ...
   ac_ieee_float<Format> &target)
{
   ...
   origtype_to_simtype(S,loc,untyped_vp, output_sc, target);
}
#endif
   ...
   ac_ieee_float<Format> &target)
{
  ...
  // now copy data
  origtype_to_simtype(S ,loc,(void*)fieldP, output_sc, target);
  return;
}
#endif
   ...
   ac_complex< ac_ieee_float<Format> > &target)
{
   ...
   // now copy REAL data
   ac_ieee_float<Format> tmp_real = target.r();
   origtype_to_simtype(S,loc,untyped_vp, output_sc, tmp_real);
   // now copy IMAG data
   ac_ieee_float<Format> tmp_imag = target.i();
   origtype_to_simtype(S,loc+1,untyped_vp, output_sc, tmp_imag);
}
#endif
   ...
   ac_complex<ac_ieee_float<Format> > &target)
{
  ...
  ac_ieee_float<Format> tmp_real = target.r();
  ac_ieee_float<Format> tmp_imag = target.i();
  origtype_to_simtype(S,loc,(void*)fieldP, output_sc, tmp_real);
  origtype_to_simtype(S,loc+1,(void*)fieldP, output_sc, tmp_imag);
  return;
}
#endif
                         ...
                         ac_ieee_float<Format>  &target)
{
  ...
  ac_ieee_float<Format> tmpIeee(tmpDouble);
  target = tmpIeee;
  return;
}
#endif
...
void catapult_to_matlab(void *uTp, 
                        mxClassID vType, 
                        int idx, 
                        const ac_ieee_float<Format>  &src)
{
  // mexPrintf("::catapult_to_matlab 5\n");
  double tmpDouble;
  ac_std_float<src.width, src.e_width> stdF = src.to_ac_std_float();
  tmpDouble = stdF.to_double();

  switch (vType)  {
  case mxLOGICAL_CLASS : {
    mxLogical *p = (mxLogical *) uTp;
    p[idx] = (mxLogical) tmpDouble;
  } break;
  case mxCHAR_CLASS : {
    mxChar *p = (mxChar *) uTp;
    p[idx] = (mxChar) tmpDouble;
  } break;
  case mxINT8_CLASS:  {
    mxInt8 *p = (mxInt8 *) uTp;
    p[idx] = (mxInt8)tmpDouble;
  } break; 
  case mxUINT8_CLASS:  {
    mxUint8 *p = (mxUint8 *) uTp;
    p[idx] = ( mxUint8)tmpDouble;
  } break; 
  case mxINT16_CLASS:  {
    mxInt16 *p = (mxInt16 *) uTp;
    p[idx] = (mxInt16)tmpDouble;
  } break; 
  case mxUINT16_CLASS: {
    mxUint16 *p = (mxUint16 *) uTp;
    p[idx] = (mxUint16)tmpDouble;
  } break; 
  case mxINT32_CLASS:  {
    mxInt32 *p = (mxInt32 *) uTp;
    p[idx] = (mxInt32) tmpDouble;
  } break; 
  case mxUINT32_CLASS: {
    mxUint32 *p = (mxUint32 *) uTp;
    p[idx] = ( mxUint32) tmpDouble;
  } break; 
  case mxINT64_CLASS:  {
    mxInt64 *p = (mxInt64 *) uTp;
    p[idx] = (mxInt64) tmpDouble;
  } break; 
  case mxUINT64_CLASS: {
    mxUint64 *p = (mxUint64 *) uTp;
    p[idx] = (mxUint64) tmpDouble;
  } break; 
  case mxSINGLE_CLASS: {
    mxSingle *p = (mxSingle *) uTp;
    p[idx] = (mxSingle) tmpDouble;
  } break; 
  case mxDOUBLE_CLASS: {
    mxDouble *p = (mxDouble *) uTp;
    p[idx] = (mxDouble)  tmpDouble;
  } break; 
  default: {
    mexErrMsgTxt("Error: Unsupported output data type.  Must be basic C data types.");
  } break;
