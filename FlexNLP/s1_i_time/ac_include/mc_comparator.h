...
template <bool issigned = false, int width = 1>
struct MaskPacket {
  static const bool _issigned = issigned;
...
template <class Tclass, class TmaskPacket>
struct mc_golden_info {
  mc_golden_info() {}
  explicit mc_golden_info(bool ignore, bool use_mask, unsigned int iteration) 
    : _use_mask(use_mask), _ignore(ignore), _iteration(iteration) {}
  mc_golden_info(const Tclass& data, bool ignore, bool use_mask, unsigned int iteration) 
    : _data(data), _use_mask(use_mask), _ignore(ignore), _iteration(iteration) {}
  Tclass    _data;
  TmaskPacket _packet;
  bool      _use_mask;
  bool      _ignore;
...
template <class Tclass, class TmaskPacket>
inline
std::ostream& operator<<(std::ostream& os, const mc_golden_info<Tclass,TmaskPacket>& gi) {
  os << gi._data;
  return os;
}
...
{    
  ...
  sc_port< tlm::tlm_fifo_get_if<mc_golden_info<Tclass,TmaskPacket> > > data_golden;
...
public:  
  ...
  mc_golden_info<Tclass,TmaskPacket>  _golden;
...
template <class Tclass, class TmaskPacket>
void mc_comparator<Tclass,TmaskPacket>::compare()
{
  while (true) {
    ...
    if (_golden._use_mask) {
      ...
      _msg.str(""); _msg <<  "Current mask (sc_lv) = " << _golden._packet._mask;
      SC_REPORT_INFO(name(), _msg.str().c_str());
      ...
      SC_REPORT_INFO(name(), _msg.str().c_str());
#endif
      //turn the data (Tclass) into a logic vector
      type_to_vector(_golden._data, _golden._packet._width, _golden._packet._data);

      //turn the dut (Tclass) into a logic vector
      type_to_vector(_dut, _golden._packet._width, _golden._packet._dut);

      //the user-defined mask value has already been converted to a sc_lv in the testbench
      _golden._packet._data &= _golden._packet._mask;
      _golden._packet._dut  &= _golden._packet._mask;

      //put these values back into the golden info
      vector_to_type(_golden._packet._data, _golden._packet._issigned, &_golden._data);
      vector_to_type(_golden._packet._dut,  _golden._packet._issigned, &_dut);
       ++_mask_compare_cnt;
#ifdef CCS_SYSC_DEBUG
    ...
    if (_golden._ignore) {
      ...
      if (_golden._use_mask) {
         ...
         _msg.str(""); _msg << "    current mask = " << _golden._packet._mask;
         SC_REPORT_INFO(name(), _msg.str().c_str());
      }
