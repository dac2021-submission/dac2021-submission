...
ifeq "$(QHOME)" ""
   $(error No Questa-Lint installation found. Check your QHOME option in Catapult)
endif
...
$(TARGET)/0in_autocheck.db: build
	...
	$(QHOME)/bin/qverify -c -od $(TARGET) -do "\
        lint methodology soc -goal release; \
        lint run -d $(TOP_DU); \
        lint generate report qlint_detail.rpt; \
        exit"
...
checkgui: $(TARGET)/0in_autocheck.db
	-@echo "============================================"
	-@echo "Launching Questa-Lint GUI on $<"
	$(QHOME)/bin/qview $(TARGET)/lint.db
...
helptext:
	@-$(ECHO) "Questa Lint Makefile"
	@-$(ECHO) "The valid targets are:"
	@-$(ECHO) "   checkgui   Compile and execute Lint in"
	@-$(ECHO) "              interactive mode"
	@-$(ECHO) "   check      Compile and execute Lint in"
	@-$(ECHO) "              batch mode"
